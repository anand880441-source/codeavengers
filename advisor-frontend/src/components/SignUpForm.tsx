...
export default function SignUpForm() {
  ...
  if (success) {
    return (
      ...
      >
        <div className="w-14 h-14 mx-auto mb-6 rounded-full bg-emerald-50 border border-emerald-200 flex items-center justify-center">
          <Check size={26} className="text-emerald-600" strokeWidth={2.5} />
        </div>
        <h2 style={{ color: 'var(--theme-text)' }} className="font-serif text-2xl font-medium tracking-tight mb-2">
          Check your email
        </h2>
        <p style={{ color: 'var(--theme-text-muted)' }} className="text-xs mb-6 leading-relaxed">
          We've sent a verification link to <strong style={{ color: 'var(--theme-text)' }}>{email}</strong>
        </p>
        <Link
          to="/signin"
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <label className="flex items-center gap-2 text-xs text-[#78716C]">
          ...
          />
          I agree to the{' '}
          <Link to="/terms" className="text-[#B2533E] hover:text-[#984330] font-semibold transition-colors">
            Terms
          </Link>{' '}
          and{' '}
          <Link to="/privacy" className="text-[#B2533E] hover:text-[#984330] font-semibold transition-colors">
            Privacy Policy
          </Link>
        </label>
