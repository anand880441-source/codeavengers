...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ conversationId, onMessageSent, onComplete }) => {
  ...
  const handleSend = async (messageContent: string | null = null) => {
        ...
        {messages.length === 0 ? (
          ...
          messages.map((msg, index) => (
            ...
            >
              <div
                className={
                  'max-w-[85%] md:max-w-[75%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed shadow-sm whitespace-pre-wrap ' +
                  (msg.role === 'user'
